...
import './App.css';
import { translations, type Language } from './assets/i18n';
...
function App() {
  ...
  const getNodes = (count: number) => {
    // Use translated steps
    const currentTranslations = translations[lang];
    const translatedSteps = allSteps.map((step, index) => ({
      ...step,
      label: currentTranslations.steps[index].label,
      description: currentTranslations.steps[index].description,
    }));

    const stepNodes = translatedSteps.map((step, index) =>
    ...
    const translatedNotes = [
      {
        ...
        position: { x: 340, y: 100 },
        color: { bg: '#f5f0ff', border: '#8b5cf6' },
        content: currentTranslations.noteContent.jsonExample,
      },
      {
        id: 'note-2',
        appearsWithStep: 8,
        position: { x: 480, y: 620 },
        color: { bg: '#fdf4f0', border: '#c97a50' },
        content: currentTranslations.noteContent.updateNote,
      },
    ];
    
    const noteNodes = translatedNotes.map((note, idx) => {
      const originalNote = notes[idx]; // Use original note structure
      const noteVisible = count >= originalNote.appearsWithStep;
  ...
  // Create edges with current language
  const createEdgeWithLang = (conn: typeof edgeConnections[0], visible: boolean): Edge => {
    return {
  ...
  const initialEdges = edgeConnections.map((conn, index) =>
    createEdgeWithLang(conn, index < allSteps.length) // Show all edges initially
  );

  const [nodes, setNodes] = useNodesState(initialNodes);
  const [edges, setEdges] = useEdgesState(initialEdges);

  // Update nodes and edges when language changes
  useEffect(() => {
    setNodes(getNodes(visibleCount));
    setEdges(edgeConnections.map((conn) =>
      createEdgeWithLang(conn, getEdgeVisibility(conn, visibleCount))
    ));
    ...
    if (visibleCount < allSteps.length) {
      ...
      setEdges(
        edgeConnections.map((conn) =>
          createEdgeWithLang(conn, getEdgeVisibility(conn, newCount))
        )
      );
    }
  }, [visibleCount, lang]);
  ...
  const handleReset = useCallback(() => {
    ...
    setNodes(getNodes(1));
    setEdges(edgeConnections.map((conn) =>
      createEdgeWithLang(conn, getEdgeVisibility(conn, 1))
    ));
  }, [lang]);
    ...
    <LanguageContext.Provider value={{ lang, toggleLanguage }}>
      <div className="app-container">
        <div className="header">
          ...
          <p><a href="https://github.com/willin/ralph" target="_blank" rel="noopener noreferrer">Github</a></p>
        ...
        <div className="controls">
          <button onClick={handlePrev} disabled={visibleCount <= 1}>
            {currentTranslations.button.previous}
          </button>
          <span className="step-counter">
            {currentTranslations.controls.stepCounter(visibleCount, allSteps.length)}
          </span>
          <button onClick={handleNext} disabled={visibleCount >= allSteps.length}>
            {currentTranslations.button.next}
          </button>
          <button onClick={handleReset} className="reset-btn">
            {currentTranslations.button.reset}
          </button>
        </div>
        <div className="instructions">
          {currentTranslations.instructions}
        </div>
      </div>
